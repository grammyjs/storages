...
await test('Pizza counter test', async () => {
  ...
  const storage = await SqlAdapter.create<JsonSessionData>('sqlite', { databasePath: ':memory:' });
...
await test('Should be changed', async () => {
  ...
  const storage = await SqlAdapter.create<string>('sqlite', { databasePath: ':memory:' });
