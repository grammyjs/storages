...
import { MinioAdapter } from '../src/minio-adapter.ts'
...
function createAdapter(prefix?: string): MinioAdapter<{ pizzaCount: number }> {
	return new MinioAdapter({ instance: client, bucket: 'test-bucket', prefix })
}

test('Should throw on missing constructor arguments', () => {
	assert.throws(() => new MinioAdapter({ bucket: 'test-bucket' } as never), /Minio Client/)
	assert.throws(() => new MinioAdapter({ instance: client } as never), /bucket/)
})
...
test('Simple string tests', async () => {
	...
	bot.use(
		session({
			initial: () => 'test',
			storage: new MinioAdapter<string>({ instance: client, bucket: 'test-bucket' }),
		})
	)
