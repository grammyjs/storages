...
import { Bot, lazySession, webhookCallback, Context, SessionFlavor } from 'grammy/web'
...
export default {
	async fetch(request: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
		...
		bot.command('haskey', async (ctx: MyContext) => {
			if (ctx.match) {
				await ctx.reply(
					(
						await new KvAdapter<SessionData>(env.MY_KV_NAMESPACE).has(ctx.match.toString())
					).toString()
				)
			} else {
		...
		bot.command('allentries', async (ctx: MyContext) => {
			try {
				for await (const entry of new KvAdapter<SessionData>(
					env.MY_KV_NAMESPACE
				).readAllEntries()) {
	...
	async scheduled(): Promise<Response> {
